...
- [](#opt-system.autoUpgrade.upgrade) can now deliberately be set to `true`.
  Before it was effectively forced to `false` when [`channel`](#opt-system.autoUpgrade.channel) was set.
  It now only defaults to be `true`
  when [`channel`](#opt-system.autoUpgrade.channel), [`flake`](#opt-system.autoUpgrade.flake), and [`storePathProvider`](#opt-system.autoUpgrade.storePathProvider) are unset.
...
- `system.autoUpgrade` gained a [`storePathProvider`](#opt-system.autoUpgrade.storePathProvider) option allowing the use of `nixos-rebuild`’s option `--store-path` on auto-upgrade. Users can declare an arbitrary executable to determine a new store path, e.g. by querying an server for the newest configuration being available. This makes it possible to outsource the configuration evaluation while systems still can update on their own.
